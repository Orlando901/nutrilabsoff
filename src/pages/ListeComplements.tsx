...
// Liste exhaustive des noms de vitamines à exclure
const vitaminesExclues = [
  'vitamine a', 'vitamine b1', 'Vitamine B2 (Riboflavine)', 'vitamine b3', 'vitamine b5',
  'vitamine b6', 'vitamine b8', 'vitamine b9', 'vitamine b12',
  'vitamine c', 'vitamine d', 'vitamine e', 'vitamine_K1',
...
export default function ListeComplements() {
  ...
  useEffect(() => {
    ...
    for (const path in modules) {
      ...
      const nomMin = fiche?.nom?.toLowerCase() || '';
      const catMin = fiche?.categorie?.toLowerCase() || '';

      const estVitamine =
        catMin.includes('vitamine') || nomMin.startsWith('vitamine');


      if (fiche && !catMin.includes('protéine') && !estVitamine) {
